...
# read in sample list 
sample_list <- read.delim(file = file.path(save_dir,paste(monkey_ID, "/", monkey_ID, "_T_samples.txt", sep="")),
                          sep = "\t", header=F)
...
plot_labels_T <- paste( paste(metadata_df[sample_index, ]$months, "m", sep = ""), metadata_df[sample_index, ]$source, metadata_df[sample_index, ]$cell_type)
...
cor_plot(my_SE[,sample_index], method_corr = "pearson", 
         ...
         plot_labels = plot_labels_T, label_size = 25, no_negatives = T)
...
all_data_pca_df$source <- my_SE[,sample_index]$source
all_data_pca_df$cell_type <- my_SE[,sample_index]$cell_type
all_data_pca_df$cell_source <- my_SE[,sample_index]$cell_type_source

timepoints <- factor(metadata_df[sample_index, ]$months)
location <- factor(my_SE[,sample_index]$source)
...
p_all <- ggplot(all_data_pca_df,aes(x=PC1,y=PC2, shape = timepoints, color = location)) + 
  ...
  scale_color_manual(values = c("PB" = "#F8766D", "BAL" = "#B79F00", "Liver" = "#00BA38", 
                                "Spleen" = "#00BFC4", "Jejunum" = "#619CFF", "Colon" = "#F564E3")) + 
  scale_shape_manual(values = c("63" = 16, "73.5" = 17, "75" = 15, "76" = 8)) + 
  theme(
    # Remove panel border
