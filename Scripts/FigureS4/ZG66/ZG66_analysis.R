...
# Modified Top 10 clones heatmap (Fold change = 2)
barcode_ggheatmap_annie_v2.1(my_SE[,sample_index], n_clones = 10, label_size = 25, 
                             plot_labels = plot_labels_NK, 
                             tissue_specific_clones = 1, foldchange_threshold = 2)
...
PB_CD56p_samples <- sample_list[3:4]
PB_DN_samples <- sample_list[5:6]
BAL_samples <- sample_list[7]
Liver_samples <- sample_list[8]
Spleen_samples <- sample_list[9:10]
...
for (i in 1:length(clones_biased_list)){
  ...
  sample_prop_sum <- data.frame(sample_prop_sum, source = my_SE$source[sample_index], 
                                ...
                                class = factor(c(rep("PB", 6), rep("Tissue", 4)), levels = c("PB", "Tissue","Sample")))
