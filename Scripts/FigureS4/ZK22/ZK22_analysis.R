...
plot_labels_NK <- paste( paste(metadata_df[sample_index, ]$Timepoint, "m", sep = ""), metadata_df[sample_index, ]$Experiment, metadata_df[sample_index, ]$Cell.Type)
...
p_all <- ggplot(all_data_pca_df,aes(x=PC1,y=PC2, shape = timepoints, color = location)) + 
  geom_point(size = 8, alpha = 0.6) + #stat_ellipse(aes(x=PC1, y=PC2,color=location),type = "norm") + 
  labs(shape = "Timepoints",color = "Tissue", x = "PC1", y = "PC2") + 
  scale_shape_manual(values = c("62" = 16, "62.5" = 17, "63" = 15)) + 
...
sample_list <- sample_list[-c(1,2)]
sample_index <- sample_index[-c(1,2)]

top_x_clone_num <- 10

PB_CD16p_samples <- sample_list[1:2]
PB_CD56p_samples <- sample_list[3]
PB_DN_samples <- sample_list[4:5]
Liver_samples <- sample_list[6:7]
Spleen_samples <- sample_list[8:9]
...
for (i in 1:length(clones_biased_list)){
  ...
  sample_prop_sum <- data.frame(sample_prop_sum, source = my_SE$source[sample_index], 
                                cell.type = my_SE$Cell.Type[sample_index], 
                                cell.type.source = my_SE$cell_source[sample_index], 
                                class = factor(c(rep("PB", 5), rep("Tissue", 4)), levels = c("PB", "Tissue", "Sample")))
  ...
  p <- ggplot(sample_prop_sum, aes(y = sample_prop_sum, x = cell.type.source, fill = class)) + 
    ...
    labs(y = "Cumulative Contribution (%)", x  = "cell type & source", title = paste(tissue, "top", top_x_clone_num, "clones",  sep = " "), 
         subtitle = paste("Top", top_x_clone_num, "clones across all samples of a particular tissue/PB subtype")) + 
